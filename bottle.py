...
class BaseRequest(object):
    ...
    def POST(self):
        ...
        if py31:
            args["fp"] = NCTextIOWrapper(args["fp"], encoding="latin1", newline="\n")
        elif py3k:
...
class HTTPResponse(Response, BottleException):
    def __init__(self, body="", status=None, headers=None, header=None, **more_headers):
        if header or "output" in more_headers:
            depr("Call signature changed (for the better). See BaseResponse")
            if header:
                more_headers.update(header)
            if "output" in more_headers:
                body = more_headers.pop("output")
...
class GeventServer(ServerAdapter):
    ...
    * See gevent.wsgi.WSGIServer() documentation for more options.
    ...
    def run(self, handler):
        ...
        if not self.options.pop("fast", None):
            wsgi = pywsgi
...
class Jinja2Template(BaseTemplate):
    def prepare(self, filters=None, tests=None, **kwargs):
        ...
        if "prefix" in kwargs:  # TODO: to be removed after a while
            raise RuntimeError(
                "The keyword argument `prefix` has been removed. " "Use the full jinja2 environment name line_statement_prefix instead."
            )
...
class StplParser(object):
    ...
    def fix_brackward_compatibility(self, line, comment):
        ...
        if parts and parts[0] in ("include", "rebase"):
            ...
            if len(parts) == 1:
                return "_printlist([body])", comment
            elif len(parts) == 2:
